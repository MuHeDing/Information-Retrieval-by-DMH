...
def read_json_file(file_name):
    ...
    with open(file_name, 'r', errors='ignore') as f:
         tweet_id_set = set()
         for line in f:
            json_obj = json.loads(line.strip())  #用于移除字符串头尾指定的字符（默认为空格或换行符）或字符序列。
...
def filter_out_of_set():
    ...
    # filer_qrels_file(in_file, out_file, tweet_id_set) #将 tweet 中 没有的id 给剔除
